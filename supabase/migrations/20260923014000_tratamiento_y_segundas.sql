alter table public.boletas_entrada add column if not exists tratamiento_cera text;
alter table public.boletas_entrada add constraint boletas_entrada_tratamiento_cera_check check (tratamiento_cera is null or tratamiento_cera in ('Sin cera','Cera orgánica','Parafina de candela'));
alter table public.boleta_rendimientos add column if not exists paga_productor boolean not null default true;
alter table public.ordenes_compra add column if not exists precio_segunda_gruesa numeric check (precio_segunda_gruesa >= 0);
alter table public.ordenes_compra add column if not exists precio_segunda_menuda numeric check (precio_segunda_menuda >= 0);
...
begin
  ...
  for v_rendimiento in select value from pg_catalog.jsonb_array_elements(coalesce(p_rendimientos,'[]'::jsonb)) loop
    ...
    insert into public.boleta_rendimientos(boleta_id,producto,calidad,presentacion_kg,cajas,kg_manual,orden_venta_linea_id,posicion_paleta,observaciones,paga_productor)
    values(v_id,v_producto,v_rendimiento->>'calidad',nullif(v_rendimiento->>'presentacion_kg','')::numeric,coalesce(nullif(v_rendimiento->>'cajas','')::integer,0),nullif(v_rendimiento->>'kg_manual','')::numeric,nullif(v_rendimiento->>'orden_venta_linea_id','')::uuid,nullif(v_rendimiento->>'posicion_paleta','')::smallint,v_rendimiento->>'observaciones',coalesce((v_rendimiento->>'paga_productor')::boolean,true));
  end loop;
  return v_id;
end;
$$;
revoke all on function private.guardar_boleta_planta(uuid,text,uuid,timestamptz,date,timestamptz,timestamptz,text,text,smallint,text,text,text,numeric,text,numeric,text,jsonb,jsonb,text) from public,anon;
grant execute on function private.guardar_boleta_planta(uuid,text,uuid,timestamptz,date,timestamptz,timestamptz,text,text,smallint,text,text,text,numeric,text,numeric,text,jsonb,jsonb,text) to authenticated;

create or replace function public.guardar_boleta_planta(
  p_boleta_id uuid,p_codigo text,p_orden_compra_id uuid,p_fecha_hora timestamptz,
  p_fecha_labor date,p_inicio_proceso timestamptz,p_fin_proceso timestamptz,
  p_turno text,p_clima text,p_linea_proceso smallint,p_encargado text,p_encargado_banda text,
  p_condicion text,p_cantidad_recipientes numeric,p_tipo_recipiente text,p_promedio_peso numeric,
  ...
  select private.guardar_boleta_planta(p_boleta_id,p_codigo,p_orden_compra_id,p_fecha_hora,p_fecha_labor,p_inicio_proceso,p_fin_proceso,p_turno,p_clima,p_linea_proceso,p_encargado,p_encargado_banda,p_condicion,p_cantidad_recipientes,p_tipo_recipiente,p_promedio_peso,p_observaciones,p_trabajos,p_rendimientos,p_tratamiento_cera)
$$;
revoke all on function public.guardar_boleta_planta(uuid,text,uuid,timestamptz,date,timestamptz,timestamptz,text,text,smallint,text,text,text,numeric,text,numeric,text,jsonb,jsonb,text) from public,anon;
grant execute on function public.guardar_boleta_planta(uuid,text,uuid,timestamptz,date,timestamptz,timestamptz,text,text,smallint,text,text,text,numeric,text,numeric,text,jsonb,jsonb,text) to authenticated;
