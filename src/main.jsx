...
const nav = [
  ...
  ['Corte semanal', CircleDollarSign], ['Cuentas por cobrar', ArrowUpRight], ['Cuentas por pagar', ArrowDownRight], ['Bancos', Landmark], ['Proveedores', Users], ['Clientes', UserRound], ['Trabajadores', Users]
]
...
function Module({title,role,search,setSearch,onNew,onEdit,refresh}){
  ...
  useEffect(()=>{let active=true;if(!table){setItems([]);return}
    ...
    const load=async()=>{
      const [result,orders,purchaseReceipts,clientsResult]=await Promise.all([supabase.from(table).select(title==='Órdenes de venta'?'*,ordenes_venta_lineas(total)':'*').order('creado_en',{ascending:false}).limit(100),title==='Boletas de entrada'?supabase.rpc('ordenes_compra_para_planta'):Promise.resolve({data:[],error:null}),title==='Órdenes de compra'?supabase.from('boletas_entrada').select('orden_compra_id,finalizada_en').limit(1000):Promise.resolve({data:[],error:null}),title==='Órdenes de venta'?supabase.from('clientes').select('id,nombre'):Promise.resolve({data:[],error:null})])
  ...
  })).filter(r=>(!['Boletas de entrada','Órdenes de venta','Órdenes de compra'].includes(title)||((title==='Órdenes de compra'?Boolean(closedPurchases[r.item.id]):Boolean(r.item.finalizada_en))===history&&(week==='all'||weekOf(r.item.fecha_hora||r.item.fecha)===week)))&&`${r.codigo} ${r.principal} ${r.detalle} ${r.estado}`.toLowerCase().includes(search.toLowerCase())),[items,search,title,receiptOrders,salesClients,closedPurchases,history,role,week])
