...
const nav = [
  ...
  ['Corte semanal', CircleDollarSign], ['Cuentas por cobrar', ArrowUpRight], ['Cuentas por pagar', ArrowDownRight], ['Bancos', Landmark], ['Proveedores', Users], ['Fincas', PackageCheck], ['Clientes', UserRound], ['Colaboradores', Users]
]
...
function Module({title,role,search,setSearch,onNew,onEdit,refresh,salesWeek,setSalesWeek}){
  ...
  const rows=useMemo(()=>items.map((item,index)=>({
    ...
    principal:title==='Órdenes de venta'?`${salesClients[item.cliente_id]||'Cliente pendiente'}${item.numero_cliente?` ${item.numero_cliente}`:''}`:title==='Boletas de entrada'?(receiptOrders[item.orden_compra_id]?.productor_nombre||item.producto||'Productor pendiente'):item.nombre||item.productor_nombre||item.producto||item.mercado||'Registro',
    detalle:title==='Órdenes de venta'?`Salida ${item.fecha_salida||'pendiente'} · Pedido recibido ${item.fecha||'sin fecha'} · ${item.mercado||'Mercado pendiente'}${item.contenedor?` · Contenedor ${item.contenedor}`:''}`:title==='Boletas de entrada'?`${item.producto||receiptOrders[item.orden_compra_id]?.producto||'Producto'} · ${receiptOrders[item.orden_compra_id]?.codigo||'Sin orden de compra'}`:title==='Órdenes de compra'?`${item.fecha||'Sin fecha'} · ${item.producto||'Producto pendiente'}${item.lugar?` · ${item.lugar}`:''}${item.boleta_campo_referencia?` · Boleta ${item.boleta_campo_referencia}`:''}`:item.tipo||item.lugar||item.finca_lugar||item.mercado||item.observaciones||'Sin detalle',
    estado:title==='Órdenes de compra'?(item.estado==='Anulada'?'Anulada':closedPurchases[item.id]?'Sellada':'Pendiente'):['Boletas de entrada','Órdenes de venta'].includes(title)?(item.finalizada_en?'Finalizada':'Pendiente'):item.estado||(item.activo===false?'Inactivo':'Activo'),
  ...
  })).filter(r=>(!['Boletas de entrada','Órdenes de venta','Órdenes de compra'].includes(title)||((title==='Órdenes de compra'?(voided?r.item.estado==='Anulada':r.item.estado!=='Anulada'&&Boolean(closedPurchases[r.item.id])===history):Boolean(r.item.finalizada_en)===history)&&(week==='all'||weekOf(title==='Órdenes de venta'?(r.item.fecha_salida||r.item.fecha):(r.item.fecha_hora||r.item.fecha))===week)))&&`${r.codigo} ${r.principal} ${r.detalle} ${r.estado}`.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>title==='Órdenes de compra'?(a.item.fecha||'').localeCompare(b.item.fecha||'')||(a.item.creado_en||'').localeCompare(b.item.creado_en||''):0),[items,search,title,receiptOrders,salesClients,closedPurchases,history,voided,role,week])
  return <><div className="modulebar"><div><p>Administre y consulte la información de {title.toLowerCase()}.</p></div>{table&&<button className="primary" onClick={onNew}><Plus size={18}/>Nuevo registro</button>}</div><section className="panel tablepanel">{['Boletas de entrada','Órdenes de venta','Órdenes de compra'].includes(title)&&<div className="load-selector"><button type="button" className={!history&&!voided?'primary':''} onClick={()=>{setHistory(false);setVoided(false)}}>Pendientes</button><button type="button" className={history&&!voided?'primary':''} onClick={()=>{setHistory(true);setVoided(false)}}>Historial de finalizadas</button>{title==='Órdenes de compra'&&<button type="button" className={voided?'primary':''} onClick={()=>setVoided(true)}>Anuladas</button>}<label>{title==='Órdenes de venta'?'Semana de salida':'Semana'}<select value={week} onChange={e=>setWeek(e.target.value)}><option value="all">Todas las semanas</option>{[...new Set([weekOf(costaRicaToday()),weekOf(addDays(costaRicaToday(),7)),week,...items.map(x=>weekOf(title==='Órdenes de venta'?(x.fecha_salida||x.fecha):(x.fecha_hora||x.fecha)))])].filter(Boolean).sort().reverse().map(x=><option key={x} value={x}>Semana {Number(x.slice(-2))} · {x.slice(0,4)}</option>)}</select></label>{title==='Órdenes de venta'&&week!=='all'&&<button type="button" onClick={()=>{const monday=mondayOf(`${week.slice(0,4)}-01-04`);setWeek(weekOf(addDays(monday,(Number(week.slice(-2))-1)*7+7)))}}>Semana siguiente →</button>}</div>}<div className="filters"><label><Search size={18}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por código, nombre o estado…"/></label><button>Todos los estados</button></div>{error&&<div className="formerror">{error} <button type="button" onClick={()=>setRetry(n=>n+1)}>Reintentar</button></div>}{loading?<div className="empty"><p>Cargando información…</p></div>:rows.length?<div className="rows">{rows.map(r=><article key={r.key}><div className="code">{r.codigo}</div><div className="who"><b>{r.principal}</b><span>{r.detalle}</span></div><span className="pill">{r.estado}</span><strong>{r.monto}</strong>{title==='Órdenes de venta'&&!r.item.finalizada_en&&['administrador','oficina'].includes(role)&&<button type="button" onClick={()=>finishOrder(r.item)} disabled={closing===r.item.id}>{closing===r.item.id?'Finalizando…':'Finalizar pedido'}</button>}{title==='Boletas de entrada'&&!r.item.finalizada_en&&!(['Ñampí','Cabeza de ñampí'].includes(receiptOrders[r.item.orden_compra_id]?.producto)&&receiptOrders[r.item.orden_compra_id]?.tipo_compra==='En pie')&&['administrador','oficina'].includes(role)&&<button type="button" onClick={()=>finish(r.item)} disabled={closing===r.item.id}>{closing===r.item.id?'Finalizando…':'Finalizar'}</button>}{title==='Órdenes de compra'&&r.item.estado!=='Anulada'&&!linkedPurchases[r.item.id]&&['administrador','oficina'].includes(role)&&<button type="button" onClick={()=>voidPurchase(r.item)} disabled={closing===r.item.id}>{closing===r.item.id?'Anulando…':'Anular'}</button>}<button className={['Clientes','Órdenes de compra','Órdenes de venta','Boletas de entrada'].includes(title)?'arrow edit-client':'arrow'} type="button" onClick={()=>onEdit(r.item)} aria-label={`Editar ${r.principal}`} disabled={!['Clientes','Órdenes de compra','Órdenes de venta','Boletas de entrada'].includes(title)||['Boletas de entrada','Órdenes de venta'].includes(title)&&!!r.item.finalizada_en||title==='Órdenes de compra'&&(!!closedPurchases[r.item.id]||r.item.estado==='Anulada')}>{['Clientes','Órdenes de compra','Órdenes de venta','Boletas de entrada'].includes(title)?'Editar':<ChevronRight/>}</button></article>)}</div>:<div className="empty"><PackageCheck size={42}/><h3>Sin registros todavía</h3><p>{['Boletas de entrada','Órdenes de venta','Órdenes de compra'].includes(title)?(voided?'No hay órdenes anuladas.':history?'Todavía no hay registros finalizados.':'No hay registros pendientes.'):table?`Puede crear el primer registro de ${title.toLowerCase()}.`:'Este módulo se conectará en la siguiente etapa.'}</p>{table&&<button className="primary" onClick={onNew}><Plus size={18}/>Crear registro</button>}</div>}</section></>
}
