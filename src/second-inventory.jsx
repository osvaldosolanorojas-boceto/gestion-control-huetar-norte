...
export default function SecondInventory(){
  const [lots,setLots]=useState([]),[sold,setSold]=useState([]),[orders,setOrders]=useState({}),[sales,setSales]=useState([]),[open,setOpen]=useState(false),[form,setForm]=useState(initial),[error,setError]=useState(''),[loading,setLoading]=useState(true),[soldLoaded,setSoldLoaded]=useState(false),[saving,setSaving]=useState(false),[showAll,setShowAll]=useState(false)
  const reload=async()=>{
    ...
    try{
      ...
      const [b,c,d]=await Promise.allSettled([supabase.from('ventas_segundas').select('*').order('creado_en',{ascending:false}).limit(1000),supabase.rpc('ordenes_compra_para_planta'),supabase.from('ventas_locales').select('*').order('creado_en',{ascending:false}).limit(100)])
