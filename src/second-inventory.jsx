...
const initial=()=>({codigo:`VL-${Date.now()}`,fecha:new Date().toISOString().slice(0,10),comprador:'',referencia:'',moneda:'CRC',observaciones:'',cobro_efectivo:false,monto_efectivo:'',lineas:[emptyLine()]})
export default function SecondInventory(){
  ...
  const save=async()=>{
    ...
    const total=totals.reduce((a,b)=>a+b,0),payment=parse(form.monto_efectivo)
    if(form.cobro_efectivo&&(!Number.isFinite(payment)||payment<=0||payment>total||Math.round(payment*100)!==payment*100)){setError('El efectivo recibido debe ser positivo y no superar el total de la venta.');return}
    setSaving(true);setError('')
    const args={p_codigo:form.codigo,p_fecha:form.fecha,p_comprador:form.comprador.trim(),p_referencia:form.referencia.trim()||null,p_moneda:form.moneda,p_observaciones:form.observaciones.trim()||null,p_lineas:lines}
    const {error:e}=await supabase.rpc(form.cobro_efectivo?'registrar_venta_local_con_efectivo':'registrar_venta_local',form.cobro_efectivo?{...args,p_cobro:payment}:args)
  ...
  return <><div className="modulebar"><div><p>Segundas y rechazo pendientes por boleta. Una venta local puede incluir varios productores, calidades y productos.</p></div><button className="primary" disabled={!soldLoaded} onClick={()=>start()}><Plus size={18}/>Nueva venta local</button></div>
    ...
    <button className="plant-add-worker local-add" onClick={()=>setForm(f=>({...f,lineas:[...f.lineas,emptyLine()]}))}><Plus size={16}/>Agregar otro producto o boleta</button><div className="receipt-calculation">Total a cobrar: <b>{money(totals.reduce((a,b)=>a+b,0),form.moneda)}</b></div><div className="formgrid"><label className="checklabel wide"><input type="checkbox" checked={form.cobro_efectivo} onChange={e=>setForm({...form,cobro_efectivo:e.target.checked,monto_efectivo:e.target.checked?String(Math.round(totals.reduce((a,b)=>a+b,0)*100)/100):''})}/> El comprador paga en efectivo ahora</label>{form.cobro_efectivo&&<label>Monto recibido en efectivo ({form.moneda})<input type="number" min="0.01" step="0.01" inputMode="decimal" value={form.monto_efectivo} onChange={e=>setForm({...form,monto_efectivo:e.target.value})}/></label>}<label className="wide">Observaciones<input value={form.observaciones} onChange={e=>setForm({...form,observaciones:e.target.value})}/></label></div>{error&&<div className="formerror">{error}</div>}<div className="modalactions"><button disabled={saving} onClick={()=>setOpen(false)}>Cancelar</button><button disabled={saving} className="primary" onClick={save}>{saving?'Guardando…':'Registrar venta local'}</button></div></div></div>}
